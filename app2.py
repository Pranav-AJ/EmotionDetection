...
# Load model.
classifier =load_model('model_50.h5')

# load weights into new model
classifier.load_weights("model50.weights.h5")
...
def main():
    ...
    activities = ["Home", "Live Face Emotion Detection", "Upload Video","About"]
    ...
    # Live Face Emotion Detection.
        ...
        * Get ready with all the emotions you can express. 
        ...
        st.write("3. When you are done, click stop to end.")
        webrtc_streamer(key="example", video_processor_factory=VideoTransformer)
    
    #Upload video
    elif choice == "Upload Video":
        st.title("Emotion Detection in Uploaded Video")
        st.write("Upload a video and see emotions displayed on faces frame by frame.")

        uploaded_file = st.file_uploader("Choose a video file", type=['mp4', 'avi'])
        if uploaded_file:
            st.video(str(uploaded_file))
    # About.
    elif choice == "About":
